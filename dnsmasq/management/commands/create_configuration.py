...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options) -> None:
        ...
        with open(options['filename'], mode='w') as file:
            ...
            for item in Interface.objects_enabled.filter(
            ...
            for item in ListenAddress.objects_enabled.order_by('order'):
                add_description(item.description)
                file.write(f'listen-address={item.address}\n')
            # Domains
            add_header('Domains')
            for item in Domain.objects_enabled.order_by('order'):
                add_description(item.description)
            ...
            for item in DhcpRange.objects_enabled.order_by('order'):
                add_description(item.description)
